...
업데이트: 2026-05-26, `main` 기준 코드 재검토 및 preprocessing worker CLI subpipeline 머지 반영

## 현재 판단

현재 NeuroWeave는 "실제 데이터로 끝까지 도는 분석 MVP"에서 "research exploration alpha 기반"으로 한 단계 올라왔다. UI hydration/CORS 안정화와 preprocessing worker CLI 분리는 완료되어 `main`에 머지되었다. 다만 worker CLI는 아직 preprocessing에만 적용되어 있고, epoching/ERP는 기존 multiprocessing worker 경로를 사용한다.
...
| preprocessing worker CLI | 완료 | `eeg_processing.worker_cli preprocessing`, API subprocess 전환, worker artifact 저장, PR #6 머지 | epoching/ERP로 패턴 확장 |
| Phase 2/3 browser smoke | 완료 | `npm run e2e:all` 추가, Phase 2 + Phase 3 epoch + ERP smoke 통과 | CI/릴리스 검증에 계속 사용 |
| epoching/ERP worker CLI | 미완료 | `apps/api/main.py`의 epoch/ERP는 아직 `multiprocessing.spawn` + result queue 기반 | 다음 subpipeline 1순위 |
| BIDS sidecar ingest MVP | 미완료 | `packages/eeg-io/src/eeg_io/bids_sidecars.py` 없음 | worker CLI 확장 후 착수 |
| Event mapping v2 | 미완료 | null 처리/row filter/condition preset 모델 없음 | BIDS events와 같이 설계 |
...
검증 기준 시점: 2026-05-26, `main` 커밋 `0965b6d Add preprocessing worker CLI contract`
...
## 다음 subpipeline: epoching/ERP worker CLI 확장
...
- epoching/ERP worker CLI 확장 시작
- worker CLI job routing 확장
- epoching API subprocess 전환

완료 기준:

- epoching run이 API 내부 multiprocessing target 직접 호출 없이 subprocess CLI로 실행됨
- 기존 epoch API 테스트 통과

### 5월 28일 목요일

- ERP worker CLI 전환
- 세 worker job 공통 artifact/exit code 흐름 정리
- 전체 `e2e:all` 통과

완료 기준:

- preprocessing -> epoch -> ERP 전체가 CLI worker 경유
- PR merge 후 `main` clean

### 5월 29일 금요일

- `packages/eeg-io/src/eeg_io/bids_sidecars.py` 추가
- `_channels.tsv`, `_eeg.json` ingest
- `RecordingMetadata` optional 확장
- channel type/status/units, line frequency, reference 반영

완료 기준:

- OpenNeuro `.set` 계열 metadata warning을 줄이거나 structured warning 후보로 분류
- registry 기존 JSON 호환성 유지

### 5월 30일 토요일

- BIDS events normalization
- `n/a`, `NA`, empty null 처리
- row filter 추가
- `psychopy`, `bids_events`, `eeglab_annotations` preset 초안

완료 기준:

- BIDS `events.tsv`를 연구용 event log로 안정적으로 변환
- condition derivation이 UI/API에서 일관되게 보임
...
3. epoching/ERP worker CLI 확장: 다음 단계
...
현재는 "research exploration alpha 기반"이 확보된 상태다. 다음 단계는 epoching/ERP까지 worker CLI 경계를 통일하는 것이다. 이 작업이 끝나야 BIDS ingest, structured diagnostics, QC, export를 안정적으로 얹을 수 있다.
