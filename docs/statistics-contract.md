...
This document defines the version 1 statistics object embedded under
`comparison_summary.json` as `statistics`. E2 implements the paired
mean-amplitude t-test path and keeps existing descriptive comparison fields
...
These files are contract fixtures. E2 implements the paired t-test path that
matches the implemented fixture and writes the unavailable shape when the
request does not include enough paired observations.
